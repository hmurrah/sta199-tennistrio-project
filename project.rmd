...
The density plots of the point distribution are consistent with the mean point values calculated earlier. Austria has a very high density of wine scores 
...
```
The density plots for the top five varieties display an interesting trend. Gewürztraminer, Pinot Gris, Pinot Noir, and Riesling varieties show more evenly distributed point values for wines whereas Grüner Veltliner's data distribution is concentrated around the 90 point value. Unlike with Austria, however, this larger density of wines around the 90 point value did not make the Grüner Veltliner the variety with the best average point value.
...
The density plots for the top five varieties and top five countries all look similar. The data is left skewed and most of the wines are less than 100 dollars. 
...
We do not have data tosupport normality as seen where the right portion of the final graph does not follow the line for normally distributed data. 
...
**Maybe do something similar with text analysis for cheap (under $25) vs expensive wines (over 100)?**
...
points to bring up: austria is the best. more expensive wines are better up to a point (need to determine this point, use a linear model)
  ...
  Our analyses showed that the highest rated wines are from Austria, there is a positive relationship between price and point ratings, and also displayed some small differences words used to describe highly rated and poorly rated wines.  
  ...
  The density plots for price and points further evidence Austria as a source of better wine. Austria clearly has a larger proportion of higher rated wines than the other top 5 countries yet at similar price ranges.
  
** write portion here about top varieties, note that the top varieties are from the top countries

  
  
  In addition to the calculation of the top countries and varieties, the linear models for predicted price both give evidence for a positive strong relationship between price and points. For example, in the glm for points given price, we calculated a p value of 1.05e-289 for the price factor. Additionally, the linear model with top countries and top variety factors gave p values of 5.56e-21 and 1.63e-56, respectively. At an alpha level of 0.05, this gives strong evidence against a null hypothesis that there is no relationship between being in the top variety or country and the point value.
  This means that when choosing a wine to maximize points awarded by wine connesuirs, it makes the most sense to choose a wine from one of these top countries and varieties. 
  


** write portion on sentiment analysis


** write portion on weighted models
