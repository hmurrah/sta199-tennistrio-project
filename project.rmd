...
We obtained this data set from Kaggle, where user zackthoutt on Kaggle had collected it by scraping data from WineEnthusiast.com. The variables that we utilized in our analysis were: `country`: The country that the wine is from, `description`: A few sentences from a sommelier describing the wine's taste, smell, look, feel, etc., `points`: The number of points WineEnthusiast rated the wine on a scale of 1-100 (though they say they only post reviews for wines that score $\geq$ 80), `price`: The cost for a bottle of the wine, `variety`: The type of grapes used to make the wine (ie Pinot Noir)

We conducted the following tests below to evaluate our hypotheses.

1. We created a linear model to predict points depending on if the wine was in a top country or not and depending on if the wine was in a top variety or not. We found sufficient evidence to suggest that wines in top countries and varieties have larger point values, while controlling for other variables such as price.  

We also found that the data had independence, linearity, and equal variance to support the use of a linear model analysis. It should be noted that the data did not display normality.
...
5. We calculated a weighted score for wines considering the number of points received and expense of the wine and then took the log of this value since our data is heavily right skewed. We created a transformed linear model to observe this relationship on a graph.
...
``` {r points_price_lm_2, message = F, warning = F, echo = F, eval = F}
m_points = glm(points ~ price, data = wine_data)
tidy(m_points)
```

```{r points_top_5_lm,  message = F, warning = F, echo = F}
...
While holding all other variables constant, a wine from one of the five top countries is expected to have a predicted point value that is 0.66 points higher than a wine from any other country and a  wine from one of the five top varieties is expected to have a predicted point value that is 1.145 points higher than a wine from any other variety.
...
Here we separated the wines by expensive (>= $200) and cheap (<= $10) for our analysis. In general, the words tannins, red, black, ripe, and berry did not show any obvious differences in frequency between the highly and poorly rated wines, and these words are also present in both expensive and cheap wines. Acidity was present in all wines, but was most common descriptor for cheaper wines by a fair margin. We know from our top wine varieties that red wine tends to be rated more highly, so we expected the cheap wines to reference white more often, however this was not the case. Both cheap and expensive wines referenced red often.

On the expensive side, we see the words oak, aging, and berry in high frequency. Perhaps it is the case that these expensive wines are more likely to be stored and aged in higher quality barrels (oak may be the preferred substance) than cheaper ones. The presence of aging makes sense since there tends to be a correlation between older wines and higher prices. It is notable that the descriptors berry, black, and ripe were frequent in expensive wines and in the five most poorly rated wines. This supports the idea that more expensive wines do not necessarily imply higher scoring wines.  
...
```{r austria_vis, message = F, warning = F, echo = F}
...
Top Countries and Varieties
  ...
  The density plots for price and points further convey Austria as a source of better wine. Austria clearly has a larger proportion of higher rated wines than the other top 5 countries yet at similar price ranges. Also, Austria's wines were generally cheaper than the other countries. 
  The density plots for top varieties showed that the Gewürztraminer wine variety had a more evenly distributed range of wines scores and prices, but a higher wine score on average. This might mean that while choosing a Gewürztraminer could give you a very highly rated wine, you could also have a lower rated wine. Whereas the less evenly distributed Grüner Veltliner variety primarily has wines with scores of at least 90.
...
Relationship Between Price and Points  
  ...
  The linear model with top countries and top variety factors gave p values of 5.56e-21 and 1.63e-56, respectively. At an alpha level of 0.05, this gives strong evidence against a null hypothesis that there is no relationship between being in the top variety or country and the point value.
  This means that when choosing a wine to maximize points awarded by wine connoisseurs, it makes the most sense to choose a wine from one of these top countries and varieties. It should be noted that the data for the linear model was not normally distributed. This makes analysis using a linear model less effective.
...
**maybe add in here the lm test with points and price that we took out
