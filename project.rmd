...
``` {r highest_average_score_by_country, echo = FALSE, warning = F, message = F, eval = F}
...
```{r highest_average_points_by_variety, warning = F, message = F, echo= FALSE, eval = F}
...
```{r points_countries_density, message = F, warning = F, echo = F}
country_counts <- wine_data %>%
    group_by(country) %>%
    count(country) %>%
    filter(n >= 100)
...
```{r points_variety_density, message = F, warning = F, echo = F}
...
```{r price_countries_density, message = F, warning = F, echo = F}
...
```{r price_variety_desnsity, message = F, warning = F, echo = F}
...
```{r points_price_lm, message = F, warning = F, echo = F}
...
ggplot(m_int_aug, mapping = aes(x = .fitted, y = .resid)) +
  ...
  labs(x = "Predicted Points", y = "Residuals") +   labs(title = "Independence and Linearity for Top Countries and Varieties Model")

ggplot(m_int_aug, mapping = aes(x = .resid)) +
  geom_histogram(bins = 15) + 
  labs(x = "Residuals") + labs(title = "Non Normal Distribution for Top Countries and Varieties Model")


ggplot(m_int_aug, mapping = aes(sample = .resid)) +
  stat_qq() + 
  stat_qq_line()+ labs(title = "Non Normal Distribution for Top Countries and Varieties Model")

```
We can assume independence for this data due to the way that the data was randomly collected. We have symmetrically distributed data around the y=0 line to support linearity. We also have equal variance because each vertical slice in the graph has a similar spread of data.
...
```{r lm_tests_2, message = F, warning = F, echo = F}
...
ggplot(m_int_aug2, mapping = aes(x = .fitted, y = .resid)) +
  geom_point() + 
  geom_hline(yintercept = 0, lwd = 2, col = "red", lty = 2) +
  labs(x = "Predicted Points", y = "Residuals", title = "No Independence or Linearity for Points and Price Model")

ggplot(m_int_aug2, mapping = aes(x = .resid)) +
  geom_histogram(bins = 15) + 
  labs(x = "Residuals", title = "Non Normal Distribution for Points and Price Model")


ggplot(m_int_aug2, mapping = aes(sample = .resid)) +
  stat_qq() + 
  stat_qq_line() + labs(title = "Non Normal Distribution for Points and Price Model")

```

We can assume independence for this data due to the way that the data was randomly collected. We have equal variance because there is a each vertical slice in the graph has a similar spread of data. 
...
Relationship Between Price and Points  
  ...
  It should be noted that in the data the first linear model showed was not normally distributed and did not have linearity. The data for the second linear model was not normally distributed. This makes analysis using a linear model less effective.
