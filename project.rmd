...
We conducted the following additional tests to evaluate our hypothesis and explore the data set. 

1. We created a linear model to predict the number of points that a wine would receive depending on whether or not it was made in a top country or not and whether it was a top variety or not.
 
 
2. We used a CLT based t test to determine whether there is a statistically significant difference between the highest rated country's average score and the average wine score for all countries.

We know that it is appropriate to use a CLT based test based after doing the following evaluation. 




5. We will calculate on average which variety of wine has the highest rating by calculating the mean scores of each variety and summarizing the data into a table. 

6. We created density plots for the point distribution for the top 5 varieties and top 5 countries. 

7. We created similar density plots for the price distribution for the top 5 varieties and top 5 countries.

8. We created model to show the relationship between a weighted value for the wine point score and its price.

8. We used text analysis on the description variable to pick out key word descriptors in order to determine if there is there a relationship between the descriptor and the score. We will visualize this relationship.
...
``` {r sentiment_analysis_all_wine, message = F, warning = F, echo = FALSE}
...
all_other_wines <- wine_data %>%
  filter(variety != "Gewürztraminer" & variety != "Grüner Veltliner" &
           variety != "Pinot Gris" & variety != "Pinot Noir" &
           variety != "Reisling")
descriptions <- tibble(txt = all_other_wines$description)
unnested_all <- descriptions %>%
  unnest_tokens(word, txt) %>%
    filter(word != "wine" & word != "flavors" & word != "palate" &
               word != "drink" & word != "aromas" & word != "offers" &
...
top_5_descriptor_counts <- top_5_descriptors %>%
  ...
  arrange(desc(prob)) %>%
  top_n(10)
top_5_descriptor_counts

all_descriptor_counts <- unnested_all %>%
  count(word) %>%
  mutate(prob = round(n/sum(n),4)) %>%
  arrange(desc(prob)) %>%
  top_n(10)
all_descriptor_counts
```
